...
var SysAlert = {
    ...
    fenceAlert: function(data) {
        ...
        // var alerturl = '/api/webservice/voice.php?method=Submit';
        // var requestData = {
        //     account: 'VM87828670',
        //     password: '2feedbd937148eeb2f32b45bb466c2b5',
        //     mobile: '13212384180',
        //     content: '编号000256病人离开院区，请注意处理',
        //     format: 'json'
        // };
        data.fence = fence;
        data.alias = tag ? (tag.alias ? tag.alias : tag.code) : data.code;
    ...
    tip: function(type, tag) {
        ...
        } else if (type == 'distanceAlert') {} else if (type == 'fenceAlert') {
            var fence = tag.fence;
            //ftype
            var ftype = 'info';
            if (tag.temp) {
    ...
    notification: function(type, data) {
        ...
        switch (type) {
            case 'moreMonitor':
                val = data.map + '_' + data.monitorId + '_' + data.outlier;
                break;
            default:
                val = data.code;
        }
